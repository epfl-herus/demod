...
def rescale_pdf(pdf, epsilon=1e-1):
    """
    ...
    if not np.all(np.logical_and(pdf.sum(axis=-1) < 1 + epsilon, pdf.sum(axis=-1) > 1 - epsilon)):
        raise ValueError('When computing CDF, the last elements of each cdfs are not all close enough to 1')
    # rescale all over the difference with one
    return np.reshape(pdf / pdf.sum(axis=-1)[:, None], (shape))



def rescale_cdf(cdf, epsilon=1e-1):
    """
    ...
    if not np.all(np.logical_and(cdf[:, -1] < 1 + epsilon, cdf[:, -1] > 1 - epsilon)):
        raise ValueError('Last elements of each arrays in cdf are not close enough to 1')
    # rescale all over the difference with one
    return np.reshape(cdf / cdf[:, -1][:, None], (shape))


def check_valid_cdf(cdf, epsilon=1e-6):
    """
    Performs checks on the validity of the cdfs and raises expressions if not
    ...
    -----
    ValueError
        if the cdf has non-valid values
    """
    # first reduces the dimension of the given cdfs
    shape = np.shape(cdf)
    cdf = np.reshape(cdf, (-1, shape[-1]))
    ...
    if not np.all(np.logical_and(cdf[:, -1] > 1.-epsilon, cdf[:, -1] < 1.+epsilon)):
        print(cdf, cdf.shape)
        raise ValueError('Last elements of each arrays in cdf are not all == 1')
