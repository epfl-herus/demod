"""Helper functions for parsing TOU surveys
"""
...
def states_to_transitions(states:States, return_duration:bool=False):
    """Converts a state array to transitions array.

    For the durations of states between the night there are different
    methods: TODO: expain methdos
    ...
    Returns:
        transitions_dict: containing the transitions
            transitions_dict['times']: the times at which the tranistion
                occur
            transitions_dict['persons']: the person or hh in the state
                array that performs the transition
            transitions_dict['new_states']: state after transition
            transitions_dict['old_states']: state before transition
    """
...
def states_to_transitions_secondary(
    primary_states:States, secondary_states:States,):
    """Converts a state array to transitions array.
...
def group_hh_transitions(primary_states, household_indexes, days_indexes, secondary_states=None):
