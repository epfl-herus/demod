...
try:
    ...
    df_hh = pd.read_csv(dir_name+ os.sep + 'daten' + os.sep + 'zve13_puf_hh.csv', sep=';')
    df_akt = pd.read_csv(dir_name+ os.sep + 'daten' + os.sep + 'zve13_puf_takt.csv', sep=';')
    df_pers = pd.read_csv(dir_name+ os.sep + 'daten' + os.sep + 'zve13_puf_pers.csv', sep=';')
except FileNotFoundError:
    raise Exception('Impossible to find the database files for Germany, if you have access to them, you must put them in \
        "GermanTOU/daten/zve13_puf_****.csv"')
...
def states_to_transitions(states, return_duration=False):
    ...
    if return_duration:
        ...
        for person in np.unique(transitions_dict['persons']):
            ...
            person_duration[-1] = person_times[0] + (len(states) - person_times[-1]) if person_first_state == person_last_state else 0
...
def get_mask_subgroup(
    only_fully_recorded_household=True, remove_missing_days=True, only_household_identical_days=True,
    quarter=None, weekday=None,
    n_residents=None, household_type=None,
    salary = None, hh_revenue=None,
    life_situation=None, hh_mean_age=None, age=None, geburtsland=None, gender=None, household_position=None,
    is_travelling=None):
    ...
    Args:
        ...
        life_situation (int, optional): (1 = Selbstständiger,Freiberufler, Landwirt,mithelfender Familienangehöriger, 2 = Angestellter, Arbeiter, Beamter,Richter, Zeit-/Berufs-soldat, Freiwilligsoziales/ökologisches/kulturelles Jahr,freiwilliger Wehrdienst, Bundesfreiwilligendienst3 = Auszubildender (auch Praktikant,Volontär) 4 = In Altersteilzeit (Arbeits-und Freistellungsphase) 5 = In Elternzeit (mit ungekündigtem Arbeitsvertrag) 6 = Schüler, Student 7 = Arbeitslos 8 = Im Ruhestand oder Vorruhestand 9 = Dauerhaft erwerbsunfähig 10 = Hausfrau/Hausmann 11 = Ausanderen Gründen nicht erwerbstätig.) Defaults to None.
