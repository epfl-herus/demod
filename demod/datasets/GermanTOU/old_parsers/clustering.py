...
#%%
def get_pers_attributies(
    gender=True, age=True, position_in_household=True, life_situation=True
):
    attributes = {}
    if gender:
        attributes['gender'] = np.array(['','man', 'woman'])[df_pers['ha3'].to_numpy()]
    if age:
        attributes['age'] = df_pers['alterx'].to_numpy()
    if position_in_household:
        attributes['position_in_household'] = np.array(
            ['','Haupteinkommensbezieher','Ehe-, Lebenspartner/-in',
            'Kind', 'Bruder/Schwester', 'Enkelkind', 'Vater/Mutter', 'Großvater/Großmutter',
            'Anders verwandt/verschwägert', 'Nicht verwandt/verschwägert'])[df_pers['ha6x'].to_numpy()]
    if life_situation:
        attributes['life_situation'] = df_pers['pb3'].to_numpy()
    return attributes

attributes = get_pers_attributies()
# %% split in depending on time
...
start_datetime = None
possible_states_labels, inv = np.unique(primary_states, return_inverse=True)
inv = inv.reshape(states.shape)
X = np.array([]).reshape((len(states), -1))
names = np.array([], dtype=str)
for split_time in times_splits:
    if start_datetime is None:
        start_datetime = split_time
        continue
    # gets the proportions of time spent in a particular states for every sample in the requeted time range
    proportions = np.asarray([np.mean(inv[:,start_datetime:split_time] == i, axis=1) for i in range(len(possible_states_labels))]).T
    # concatenate in the data array
    X = np.c_[X, proportions]
    # concatenates the label of each feature of X
    names = np.r_[names, np.char.add(np.array(possible_states_labels, dtype=str), np.array(['_'+str(start_datetime/6) + '-'+str(split_time/6) for i in possible_states_labels]))]
    start_datetime = split_time

# adds the attributes of the socioenomical
X = np.c_[df_pers.to_numpy()[df_akt_index_person,4:-2], X]
names = np.r_[df_pers.columns[4:-2], names]

# %%
map_size = 12
som = sompy.SOMFactory.build(X, mapsize=(map_size,map_size), component_names=names)
som.train(n_job=10,  verbose='info', train_rough_len=100)


#%%
v = sompy.mapview.View2D(map_size, map_size, 'test',text_size=8)
# could be done in a one-liner: sompy.mapview.View2DPacked(300, 300, 'test').show(som)
v.show(som, what='codebook', which_dim=[i for i in range(0,20)], cmap=None, col_sz=6,denormalize=True) #which_dim='all' default
#%%

kmeans = KMeans(n_clusters=22)
cl_labels = kmeans.fit_predict(
    som._normalizer.denormalize_by(som.data_raw,
                                    som.codebook.matrix))

som.cluster_labels = cl_labels

v.show(som, what='cluster')

# %%
u = sompy.umatrix.UMatrixView(map_size , map_size , 'umatrix', show_axis=True, text_size=8, show_text=True)
...
for attr in get_pers_attributies().values():
    attr[df_akt_index_person]
    for cluster in np.unique(cluster_id):
        print(count(attr[df_akt_index_person][cluster_id == cluster]))
...
main_activity_states, main_activity_labels = convert_states(
    primary_states[mask_subgroup], GTOU_label_to_CREST_act)
sec_activity_states, sec_activity_labels = convert_states(
    secondary_states[mask_subgroup], GTOU_label_to_CREST_act)

for desired_act in main_activity_labels:

    if desired_act not in ['Act_TV','Act_Cooking','Act_Laundry','Act_WashDress','Act_Iron','Act_HouseClean']:
        continue
